...
!
! !INTERFACE:
subroutine readMOD10A1V61obs(source)
! 
! !USES:   
  ...
  use LVT_histDataMod
  use map_utils
  use MOD10A1V61_obsMod, only : mod10a1v61obs
...
! !DESCRIPTION: 
!  
!  This subroutine provides the data reader for (resampled) MOD10A1V61 snowcover 
!  data - NDSI Normalized Difference of Snow Index. The routine also spatially
!  interpolates the MOD10A1V61 data to the model (LIS) output grid 
!  and resolution. 
!  NDSI Values are from 0 to 255 (0-100 snow, 100-255 cloud cover)
!  NOTES: 
!   The MOD10A1V61 output is available daily. 
! 
! !FILES USED:
!
! !REVISION HISTORY: 
...
!EOP
  ...
  if(mod10a1v61obs(source)%startflag.or.alarmCheck) then 
     ...
     if(file_exists) then
        ...
        if(ier.eq.0) then 
           call LVT_verify(nf90_inq_varid(ftn,'NDSI_Snow_Cover',scfid),&
                'nf90_inq_varid failed for NDSI_Snow_Cover')

           lat1 = mod10a1v61obs(source)%gridDesci(4)
           lon1 = mod10a1v61obs(source)%gridDesci(5)
           r1 = nint((lat1+59.995)/0.01)+1
           c1 = nint((lon1+179.995)/0.01)+1

           call LVT_verify(nf90_get_var(ftn,scfid,snfrac1, &
                   start=(/c1,r1/),&
                   count=(/mod10a1v61obs(source)%modis_nc, mod10a1v61obs(source)%modis_nr/)),&
                   'Error in nf90_get_var: NDSI_Snow_Cover')
        ...
        call interp_mod10a1v61(source, &
             mod10a1v61obs(source)%modis_nc, mod10a1v61obs(source)%modis_nr, &
             snfrac1, lb, snfrac)
...
end subroutine readMOD10A1V61obs
...
!
! !INTERFACE: 
subroutine interp_mod10a1v61(source, nc, nr, var_input, lb, var_output)
! 
! !USES:   
  use LVT_coreMod
  use MOD10A1V61_obsMod, only : mod10a1v61obs
...
! !DESCRIPTION: 
! 
!   This subroutine spatially interpolates the MOD10A1V61 variable to the
!   model (LIS) output grid and resolution, using a bilinear interpolation
!   approach. 
! 
!   The arguments are: 
!   \begin{description}
!    \item[nc]      number of columns in the input (MOD10A1V61) grid
!    \item[nr]      number of rows in the input (MOD10A1V61) grid
...
!EOP
  ...
  if(LVT_isAtAfinerResolution(0.01)) then
     call bilinear_interp(LVT_rc%gridDesc,lb, var_input,&
          ...
          mod10a1v61obs(source)%rlat,mod10a1v61obs(source)%rlon,&
          mod10a1v61obs(source)%w11,mod10a1v61obs(source)%w12,&
          mod10a1v61obs(source)%w21,mod10a1v61obs(source)%w22,&
          mod10a1v61obs(source)%n11,mod10a1v61obs(source)%n12,&
          mod10a1v61obs(source)%n21,mod10a1v61obs(source)%n22,LVT_rc%udef,iret)
     
  else
     call upscaleByAveraging(&
          mod10a1v61obs(source)%modis_nc*mod10a1v61obs(source)%modis_nr, &
          LVT_rc%lnc*LVT_rc%lnr, LVT_rc%udef, &
          mod10a1v61obs(source)%n11, lb, &
          var_input, lo, go)
  endif
...
end subroutine interp_mod10a1v61

!BOP
! 
! !ROUTINE: create_mod10a1v61obs_filename
